...
1. ~~**60/30/10 배분을 잰 적이 없다.**~~ → **쟀다** (2026-07-30, SSOT 14절). 읽는 화면 0.17~1.1%,
   primary CTA 가 보일 때 최대 7.6%. 10 은 상한이고 이미 그 아래라 손댈 것이 없다.
2. ~~**press 피드백이 opacity 뿐** — 스킬은 `scale(0.97)` 을 본다.~~ → **닫혔다**: 재보니 문제는
   촉감이 아니라 **밝은 표면에서 피드백이 없다**는 것(흰 카드 + opacity 0.85 = 1/255).
   `colors.surfacePressed` 로 값을 바꿔 해결했고, scale 은 쓰지 않는다(이유는 SSOT 14절).
