...
- **SECTION 자식의 `x`/`y` 는 섹션 기준 상대좌표다.** `node.x = section.x + 80` 은 틀렸다. `node.x = 80` 이 맞다. 중첩 섹션도 같다(부모 기준 상대).
- **검증은 `absoluteBoundingBox` 로만.** `node.x` 와 `section.x` 를 직접 비교하면 좌표계가 섞여 이탈을 놓친 채 통과한다(실제로 그렇게 오검증했다).
- 컨테이너를 키운 뒤 형제 섹션과 겹치는지 확인한다.
- **`UI` 페이지는 내비게이션 계층 트리다** — 부모 SECTION = 탭(+인증), 그 안에 화면 섹션을 라우트 depth(tier) 순으로 중첩. `인증` 은 맨 위 행, 탭 4개는 아래 행. 같은 tier = 같은 rel Y. 상세는 핸드오프 §3.4.
- **⚠️ 섹션 `appendChild`(reparent)는 Pretendard 텍스트를 포함하면 실패한다** ("unloaded font"). 화면 섹션을 옮기려면 §3.1 폰트 다운/복구 사이클을 먼저 돌린다. 단순 이동(`s.x=` / `resize`)은 폰트 로드 불필요.
