...
## 0. 이 스킬을 쓰는 시점 — 기본 흐름을 덮어쓰지 않는다

이 저장소의 **평소 흐름은 `main` 직접 작업 + 즉시 push** 다. 단일 작업을 하러 온 세션이 이 절차를 끌어다 쓰면 오버헤드만 는다.

| 상황 | 방식 |
|------|------|
| 작업 1건, 다른 세션 없음 | **워크트리 없이** main(또는 단일 feature 브랜치)에서 그대로 작업 |
| 예상 작업 시간 **30분 미만** | 워크트리 없이 진행 — 아래 고정비가 작업 자체보다 크다 |
| 사용자가 "병렬", "워크트리", "따로 빼서" 신호를 줌 | 이 스킬 전체 적용 |
| 다른 세션이 이미 워크트리로 작업 중(레지스트리에 활성 행 존재) | 이 스킬 전체 적용 — 새 작업도 워크트리로 격리 |

### 워크트리 1개의 고정비 (2026-07-28 드라이런 실측)
...
그래서 기준은 **30분**이다. 30분 미만이면 격리 이득보다 고정비가 크므로 그냥 main 에서 한다. 반대로 **반나절짜리 독립 슬라이스 작업 둘을 실제로 동시에 굴릴 때** 이 절차가 값을 한다.

- 시간 추정이 애매하면 워크트리를 만들지 말고 시작한다. 하다 보니 길어지면 그때 `git stash` → 워크트리로 옮기면 되고, 반대(만들어놓고 5분 만에 끝남)는 되돌릴 수 없다.
- 30분 미만이라도 **다른 세션이 같은 파일을 건드리는 중**이면 시간과 무관하게 격리한다 — 그건 비용이 아니라 사고 방지다.

위 표의 2·3·4행에 해당하면 §2 흐름을 처음부터 밟는다.
...
1. **레지스트리가 SSoT** — 충돌 판정/현황은 항상 `REGISTRY.md` 기준. 쓰기 전 재-read.
2. **베이스는 origin/main** — EnterWorktree 기본값이 맞다. release 는 베이스로 쓰지 않는다.
3. **머지는 ff-only, PR 없음** — 거부되면 자동 fallback 말고 보고.
4. **release 는 사용자 승인 후에만** — 시각 변경은 실물 렌더 확인이 선행 조건.
5. **워크트리 부팅 3종**(`.env.local`·node_modules·포트) 을 빼먹으면 검증 단계에서 시간을 다 잃는다.
6. **정리는 raw git** — ExitWorktree 는 같은 세션 한정. junction 은 `rmdir`. 레지스트리 행 삭제도 잊지 말 것.
7. **계획서는 「정리」 단계에 삭제** — 머지 후 수정 대비.
8. **self-review 먼저, 그 다음 사용자 확인** — typecheck green ≠ 화면이 맞음.
9. **커밋에 `Co-Authored-By` 금지**, 한국어 커밋 본문.
10. **프로젝트 고정 제약**: 지도·길찾기는 **카카오 전용**(구글·네이버 옵션 노출 금지), 데모/시드 데이터는 **부산 실주소**, `Alert.alert` 는 web 에서도 전역 패치로 동작하므로 web 분기 불필요.
11. **범위 제외**: `docs/roadmap/03_field-redefinition.md`(현장 → 점 자산 재정의, ERD v3급)는 2학기 과제다. 병렬 작업 후보로 올리지 않는다.
12. **OS**: PowerShell 이 주력이나 `git`/`gh` 는 Bash 도구로 실행. 경로 스니펫은 Bash 기준이고, junction 생성만 PowerShell/cmd 다.
