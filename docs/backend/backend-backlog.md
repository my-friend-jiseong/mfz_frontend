...
**먼저 확인이 필요하다 — 30일 경과분을 지우는 배치가 실재하는가?** 프론트에서는 알 수 없다.

없다면 둘 중 하나다.

1. **삭제 배치 구현** — 외근 종료 시각 기준 30일 경과분 파기. 파기 대상에 **스토리지 객체(MinIO)**
   가 포함돼야 한다. DB row 만 지우고 사진 파일이 남으면 파기한 게 아니다.
2. **문서를 실제에 맞게 수정** — 팀이 본문을 고친다.

어느 쪽이든 **8/3 시행 전에 정해져야 한다.** 지키지 못할 보관기간을 공개 문서로 선언하는 것은
단순 문구 불일치보다 무겁다.

> 참고: 보고서·외근 이력은 현재 앱에서 기한 없이 조회된다. 30일 파기가 실제로 돌면
> **오래된 보고서가 사라지는 UX 변화**가 생긴다 — 이건 문서를 고치는 쪽으로 기울 이유가 될 수 있다.
> 결정 전에 "무엇을 30일로 볼 것인가"(위치 좌표만인지, 외근 기록 전체인지)를 좁히는 게 낫다.
...
- **(D) 는 선반영하지 않았다.** 문서에 아직 버전(시행일)이 붙어 있지 않아, 지금 프론트가
  버전 문자열을 만들어 보내면 **사실이 아닌 동의 이력**이 쌓인다. 없는 것보다 나쁘다.
  문서에 시행일이 확정되는 즉시 `SignupBody` 에 optional 필드로 배선한다.
