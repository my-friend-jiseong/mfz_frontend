...
>
> ⏸️ **2026-08-29 재검증 — 변화 없음.** `curl /location-terms` 여전히 404, `/terms`·`/privacy`
...
## 31. 🟠 업로드된 파일이 전부 404 — 사진·Word 를 다시 받을 수 없다 (A ✅ 해소 · B 잔존)

> **2026-08-29 재검증(운영, 실계정)**: **(A) 사진은 이제 받힌다.** `GET /api/reports/118` 이 준
> `/storage/reports/105/1787047916155-…jpg`(**업로드 시각 2026-08-14**)를 인증 없이도 200 +
> 실제 JPEG 바이트(640×480)로 돌려받았다 — 이 파일은 2026-08-18 배포(카카오 라우팅 API 결과보고서
> 반영분)를 **이미 한 번 넘긴 뒤에도 살아있다**, 즉 "배포마다 디스크가 초기화된다"던 원인이 실제로
> 고쳐졌다는 증거다(당시 문서가 요구한 검증 기준 — "다음 배포 후에도 200인가" — 를 그대로 충족).
> **(B) `/output/*.docx` 는 그대로다.** `curl -D-` 결과 `Content-Type: text/html`, `Content-Length: 171`
...
~~🔴 **높음.** 제품 핵심(현장 전·중·후 사진)이 **재조회 불가**다.~~ → **(A) 해소로 강등.**
남은 건 (B) Word 다운로드 단독 — 🟠 **중상.** 기말 발표에서 사진 표시는 이제 라이브로
보여줄 수 있다. Word 다운로드만 여전히 실패한다(PDF export(`§19`)는 별개 경로라 무사 —
Word 를 못 받아도 PDF 로 대체 시연 가능).

### 프론트 현황

- **프론트 버그가 아니다** — URL 절대화(`src/api/config.ts:toAbsoluteFileUrl`)는 정상 동작하고,
  (B) 의 절대화된 URL 을 서버가 404 로 돌려준다.
- **선조치 후보 (미착수, B 한정)**: Word 다운로드 404 시 플레이스홀더·안내 문구. 지금은
  버튼을 눌러도 반응이 없다(§ 프론트 요청 3번 회신에 따라 문구가 갈려서 대기 중).
...
- **2026-08-29**: **§30-B 재검증 — 변화 없음.** `/location-terms` 404, `/terms`·`/privacy`
  ...
  불일치). 오늘 재확인한 §33·§31 은 그 사이 조치됐는데 B 만 그대로다.
- **2026-08-29**: **§31 부분 해소 확인.** 실계정(`abc@abc.com`)으로 로그인해 실제 보고서
  사진 URL(`/storage/reports/…`, 업로드 2026-08-14)을 재조회 — 200 + 실제 JPEG 반환. 이
  파일이 2026-08-18 배포를 넘기고도 살아있어, "배포마다 디스크 초기화" 원인이 실제로
  고쳐졌다고 판단(§31 이 요구한 검증 기준 충족). 반면 `/output/*.docx` 는 2026-07-30 실측과
  바이트 단위로 동일한 404(Express 기본, 라우트 없음) — Word 다운로드만 그대로 죽어있다.
  §31 을 (A 해소 · B 잔존)으로 갱신, 우선순위 🔴→🟠 강등.
