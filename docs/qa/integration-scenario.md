...
4. **위치 권한 거부**: 촬영은 진행, "위치를 확인할 수 없어요" 폴백 시트 진입 (에러 alert 아님).
5. **현장 0개 계정**: 폴백 시트 대신 "등록된 현장이 없습니다" → [새 현장 등록] → `/fields/new`.
6. **목록 스코프**: 현장 탭에 방문일 필터(최근 7일 등) 적용 상태에서도 매칭 후보는 **전체 현장** 기준 (`visitDateScope: 'all'` + **페이지 순회** — 21번째+ 현장도 매칭 대상) + 등록 후 화면 필터 상태 유지(미오염). 목록 조회 **실패** 시엔 부분 목록 기준 자동 매칭을 하지 않고 수동 선택 폴백("현장 목록을 모두 불러오지 못했어요").
7. **가드**: 업로드 중 백드롭·닫기·후보 변경 모두 비활성 / 업로드 실패 시 alert 후 시트 유지 → 재촬영 없이 재시도 / 갤러리 경로는 자동 매칭 미적용(기존 현장 상세 "사진 추가" 회귀 없음) / 체크인 전·중·후 슬롯 회귀 없음(phase 미지정 확인).
8. (web) [촬영] 은 파일 선택기로 동작 — 동작 확인 수준이면 충분 (1차 타깃 모바일).
...
**실기기(APK) 전용**
- [ ] 현장 탭·방문할 현장 선택에서 **데모 현장 30개 전부** 노출 + 마지막 카드까지 스크롤 + 바닥 CTA 노출
      ("끝까지 스크롤 안 됨" 버그의 실제 원인은 listMine 1페이지 절단(20개) — `listMineAll` 페이지
      순회로 수정(2026-06-05). enableDynamicSizing=false·flex:1 은 무관했으나 하이진으로 유지.
      검증은 개수 카운트가 결정적: 30개 미만이면 회귀)
...
- [ ] S9 safeBack 폴백
- [ ] S10 토큰 회전
- [ ] S11 Quick Photo — 최근접 매칭·폴백·스코프 미오염 (단위: `npm test`)
