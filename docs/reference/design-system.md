...
- **텍스트 스타일 25 개** — 9 개는 `typography.ts` 키와 1:1. 코드는 굵기를 `<Text weight>` prop 으로 받지만 Figma 엔 그 축이 없어, 컴포넌트가 실제로 쓰는 조합만 `body-bold` · `bodySm-semibold` · `caption-bold` · `groupLabel` 등으로 추가했다. 문서 가구는 `docs/*` 로 분리.
...
- **화면은 `UI` 페이지** — `DesignSystem` 은 토큰·컴포넌트 전용이고, 화면 프레임은 `UI` 페이지의 `N_화면이름` 섹션에 넣는다. 폭 390, 높이는 스크롤 전체(뷰포트 844 는 캡션으로 표기).
