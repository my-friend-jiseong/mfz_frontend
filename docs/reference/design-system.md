...
- **변수 158 개** — Primitives / Color / Spacing / Typography / Motion 5 컬렉션. 문서 스와치까지 전부 바인딩돼 토큰을 고치면 따라 움직인다.
...
- **불투명도 단위 주의** — Figma 의 `OPACITY` 스코프 변수는 **% 단위(0–100)** 다. `opacity.disabled` 0.4 는 Figma 변수 `40` 으로 넣어야 한다. 0.4 를 그대로 넣으면 실제 불투명도가 0.004 가 되어 거의 투명해진다.
- **duration 은 scope 를 비웠다** — Figma 에 바인딩 대상이 없어 픽커에 뜨면 잡음만 된다. 참조용 값이다. easing 은 변수로 만들지 않고 섹션에 곡선으로만 남겼다.
- **아이콘** — `node_modules` 의 `Ionicons.ttf` 에서 윤곽선을 직접 추출했다(다운로드 아님). 컴포넌트 이름은 `icon/<name>`, 24×24, fill 은 `text/default` 바인딩. Button · FilterChip · Input 의 아이콘 자리는 `INSTANCE_SWAP` 이라 드롭다운으로 갈아끼운다.
