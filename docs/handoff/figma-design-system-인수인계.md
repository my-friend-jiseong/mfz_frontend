...
> **기준일** 2026-08-29 · `main` `13865df` 기준. Figma 인벤토리는 이 날짜에 플러그인 API 로 **직접 실측**했다.
...
토큰·타이포·아이콘·컴포넌트 라이브러리를 만들었다. **거의 다 끝났고 미결 3건이 남았다**(§4).
...
| 상태 배지 | `94:2` | **5185** | **-105** | 807 | 934 |

⚠️ **아이콘 ↔ 상태 배지가 겹쳐 있다.** 이게 P0 미결이다(§4.1).
...
`아이콘`(5100..6460 × -105..1085)과 `상태 배지`(5185..5992 × -105..829)가 겹친다.
`컴포넌트` 섹션을 4020px 로 키우면서 `상태 배지` 를 자동으로 밀었는데, 밀린 자리가 하필 `아이콘` 위였다.

**권장 위치**: `상태 배지` → `x=5100, y=1300`. `아이콘` 아래(y 1085 종료)이고 `컴포넌트`(x 4985 종료) 오른쪽이라 둘 다 비껴간다.
...
고친 뒤 §6 검증 스크립트로 `ov: []` 를 확인한다.

### 4.2 P1 — `docs/reference/design-system.md` §15 갱신 (문서가 코드보다 뒤처져 있음)

현재 §15 는 **틀린 수치**를 담고 있다. 고쳐야 할 것:
...
배리언트 총합은 재계산할 것(§6 스크립트가 세트별 개수를 뱉는다).
**§15 를 늘리지 말 것** — 사용자가 장황함을 네 번 지적했다. 수치만 정정하고 새 컴포넌트는 한 줄로 붙인다.

### 4.3 P2 — 커밋 + 푸시

미커밋 상태로 남아 있다(마지막 커밋 `13865df` 이후 아무것도 안 올라갔다):

```
 M docs/reference/design-system.md      # §9 ErrorState 행 + §10 ErrorState 행 (2줄 추가)
 ?? docs/일가요_앱배너.png               # 추적 안 됨 — 커밋 여부는 사용자에게 확인
```

여기에 P1 의 §15 수정과 이 인수인계 문서가 더해진다.
...
2026-08-29 기준 이 스크립트는 **`ov` 만 빼고 전부 통과**한다(`outside 0`, 폰트 `{Pretendard: 661}`, `maxLen 39`, `totalVars 170`).
